...
class HsLaporanTransaksi extends Controller
{
    ...
    public function rptpersediaan()
    {
        ...
        $golongan = M_golongan::get();
        return view('history.laporantransaksi.rptpersediaan')->with(['golongan' => $golongan, 'periode' => $periode]);
    }
    public function laporanpersediaan(Request $request)
    {
        ...
        if ($request->kriteria == "golongan") {
            if ($request->idgolongan == "all") {
                ...
                //     ->orderBy('namagolongan', 'asc')
                //     ->get();
                $datastok = M_barang::select('barang.*', 'golongan.namagolongan')
                    ->join('golongan', 'barang.idgolongan', '=', 'golongan.idgolongan')
                    ->with(['jmlstokhistory' => function ($query) use ($request) {
                        $query->where('detailstokopname.kodeopname', $request->kodeopname);
                    }])
                    ->orderBy('namagolongan', 'asc')
                    ->get();
            ...
            return view('history.laporantransaksi.laporanpersediaanpergolongan')->with(['datastok' => $datastok, 'tglmulai' => $tglmulai, 'tglakhir' => $tglakhir]);
        }
    }
