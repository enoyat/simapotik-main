...
Public Class FormKelontong
    ...
    Private Sub txtkdbarang_KeyDown(sender As Object, e As KeyEventArgs) Handles txtkdbarang.KeyDown
        ...
        ElseIf (e.KeyCode = Keys.F2) Then
            FormBarangKelontong.Close()
            FormBarangKelontong.ShowDialog()
        End If
    End Sub
    ...
    Private Sub PD_PrintPage(sender As Object, e As PrintPageEventArgs) Handles PD.PrintPage
        ...
        'e.Graphics.DrawString("NOTA", f14, Brushes.Black, centermargin, 5, tengah)
        ...
        e.Graphics.DrawString("NOTA", f10, Brushes.Black, 0, 60)
        ...
        e.Graphics.DrawString("Terima kasih", f10, Brushes.Black, 0, 90 + tinggi)
    ...
    Public Function SendJsonToApi(apiUrl As String, jsonData As String, ByRef responseContent As String) As Boolean
        ...
        Try
            ...
            Using client As New HttpClient()
                ...
                If responseContent = "" Then
                    MessageBox.Show("Response API kosong")
                    Return False
                End If

                Dim json = JObject.Parse(responseContent)

                Dim status As String = ""

                If json("status") IsNot Nothing Then
                    status = json("status").ToString()
                End If

                If status = "success" Then

                    If json("id_penjualan") IsNot Nothing Then
                        txtnonota.Text = json("id_penjualan").ToString()
                    End If

                    Return True

                ElseIf status = "stok_tidak_cukup" Then

                    Dim pesan As String = "Stok tidak cukup :" & vbCrLf

                    If json("items") IsNot Nothing Then
                        For Each item In json("items")

                            pesan &= "Barang ID " & item("kdbarang").ToString() &
                             " stok tersedia " & item("stok").ToString() &
                             " diminta " & item("diminta").ToString() & vbCrLf

                        Next
                    End If

                    MessageBox.Show(pesan, "Stok Tidak Cukup")
                ...
                Else
                    ...
                    If json("message") IsNot Nothing Then
                        message = json("message").ToString()
                    End If

                    MessageBox.Show("Error API : " & message)
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        FormCustomer.Close()
        FormCustomer.caller.Text = "kelontong"
    ...
    Private Sub txtjmltotal_TextChanged(sender As Object, e As EventArgs) Handles txtjmltotal.TextChanged
