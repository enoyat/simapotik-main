...
class HsPembelian extends Controller
{
    public $total;
    ...
    public static function kekata($x)
    {
        ...
        if ($x < 12) {
            ...
            $temp = HsPembelian::kekata($x - 10) . " belas";
        } else if ($x < 100) {
            $temp = HsPembelian::kekata($x / 10) . " puluh" . HsPembelian::kekata($x % 10);
        } else if ($x < 200) {
            $temp = " seratus" . HsPembelian::kekata($x - 100);
        } else if ($x < 1000) {
            $temp = HsPembelian::kekata($x / 100) . " ratus" . HsPembelian::kekata($x % 100);
        } else if ($x < 2000) {
            $temp = " seribu" . HsPembelian::kekata($x - 1000);
        } else if ($x < 1000000) {
            $temp = HsPembelian::kekata($x / 1000) . " ribu" . HsPembelian::kekata($x % 1000);
        } else if ($x < 1000000000) {
            $temp = HsPembelian::kekata($x / 1000000) . " juta" . HsPembelian::kekata($x % 1000000);
        } else if ($x < 1000000000000) {
            $temp = HsPembelian::kekata($x / 1000000000) . " milyar" . HsPembelian::kekata(fmod($x, 1000000000));
        } else if ($x < 1000000000000000) {
            $temp = HsPembelian::kekata($x / 1000000000000) . " trilyun" . HsPembelian::kekata(fmod($x, 1000000000000));
        }
        return $temp;
    }
    ...
    {
        if ($x < 0) {
            $hasil = "minus " . trim(HsPembelian::kekata($x));
        } else {
            $hasil = trim(HsPembelian::kekata($x));
        }
        switch ($style) {
            case 1:
    ...
    public function index()
    {
        ...
        return view('history.pembelian.index')->with(['mstransaksis' => $mstransaksis, 'barang' => $barang, 'kdtransaksi' => $kdtransaksi, 'supplier' => $supplier, 'lokasi' => $lokasi]);
    }
    ...
    public function cartview()
    {
        ...
        return view('history.pembelian.cartview')->with(['data' => $data]);
    }
    public function carthapus(Request $request)
    {
        ...
        Session::put('cart', $cart);
        return view('history.pembelian.cartview');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                DB::commit();
                return Redirect()->route('history.pembelian.invoice');
    ...
    public function invoice()
    {
        ...
        return view('history.pembelian.invoice')->with(['pembelian' => $data, 'datapembelian' => $datapembelian]);
    }

    public function batal()
    {
        return view('history.pembelian.batal');
    }
    ...
    public function detail($id)
    {
        ...
        return view('history.pembelian.create')->with(['datajurnal' => $datajurnal]);
    }
    public function destroy($id)
    {
        ...
        return redirect()->route('history.pembelian.retur')
    ...
    public function trpembelian()
    {
        ...
        return view('history.pembelian.trpembelian')->with(['pembelian' => $pembelian]);
    }
    public function trdetail($id)
    {
        $data = M_hsdetailpembelian::where('idhspembelian', $id)
            ->with('get_barang')->get();
        return view("history.pembelian.detail")->with('data', $data);
    }
    public function trreturdetail($id)
    {
        ...
        return view("history.pembelian.returdetail")->with('data', $data);
    }
    ...
    public function caribarang(Request $request)
    {
        ...
        return view('history.pembelian.formbarang', ['barang' => $barang, 'jenisharga' => $request->jenisharga]);
        //
    }
    public function fetch(Request $request)
    {
        ...
        return view('history.pembelian.fetch', ['barang' => $barang]);
        //
    }
    public function retur()
    {
        $periode = MTrstokopname::get();
        return view('history.pembelian.retur', ['periode' => $periode]);
    }

    public function fetchretur(Request $request)
    {
        if ($request->kriteria == "nofaktur") {
            $pembelian = M_hspembelian::join('supplier', 'hspembelian.idsupplier', '=', 'supplier.idsupplier')
                ->where('hspembelian.nofaktur', $request->keyword)->orderBy('id', 'DESC')
                ->where('f_statustransaksi', '0')
                ->where('hspembelian.kodeopname', $request->kodeopname)
                ->get();
        } else if ($request->kriteria == "nobatch") {
            $pembelian = M_hspembelian::join('supplier', 'hspembelian.idsupplier', '=', 'supplier.idsupplier')
                ->join('hsdetailpembelian', 'hspembelian.idhspembelian', '=', 'hsdetailpembelian.idhspembelian')
                ->where('hsdetailpembelian.nobatch', $request->keyword)->orderBy('hspembelian.id', 'DESC')
                ->where('f_statustransaksi', '0')
                ->where('hspembelian.kodeopname', $request->kodeopname)
                ->select('hspembelian.*', 'supplier.namasupplier')
                ->get();
            ...
            $pembelian = M_hspembelian::join('supplier', 'hspembelian.idsupplier', '=', 'supplier.idsupplier')
                ->join('hsdetailpembelian', 'hspembelian.idhspembelian', '=', 'hsdetailpembelian.idhspembelian')
                ->join('barang', 'hsdetailpembelian.kdbarang', '=', 'barang.kdbarang')
                ->where('hspembelian.id', 'like', '%' . $request->keyword . '%')->orderBy('id', 'DESC')
                ->where('f_statustransaksi', '0')
                ->where('hspembelian.kodeopname', $request->kodeopname)
                ->select('hspembelian.*', 'supplier.namasupplier')
                ->get();
        ...
        return view('history.pembelian.fetchretur', ['pembelian' => $pembelian]);
        //
    }
    public function listretur($id)
    {
        $data = M_returpembelian::with('get_pembelian')->where('idpembelian', $id)->where('f_statustransaksi', '0')->get();
        return view('history.pembelian.listretur')->with(['retur' => $data, 'idpembelian' => $id]);
    }
    public function inbond($id)
    {
        ...
        return view('history.pembelian.inbond')->with(['pembelian' => $data, 'datapembelian' => $dataorder, 'lokasi' => $lokasi, 'kdtransaksi' => $kdtransaksi]);
    }
    public function inretur($id)
    {
        ...
        return view('history.pembelian.inretur')->with(['pembelian' => $data, 'datapembelian' => $dataorder, 'lokasi' => $lokasi, 'kdtransaksi' => $kdtransaksi]);
    }
    public function approveretur(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                Session::put('id', $lastid);
                DB::commit();
                return Redirect()->route('history.pembelian.invoiceretur');
            } catch (\Exception $e) {
                Alert::error('Error', $e->getMessage());
                DB::rollback();
    ...
    public function formretur(Request $request)
    {
        ...
        return view('history.pembelian.formretur', ['barang' => $barang, 'idrecord' => $request['idrecord']]);
    }
    public function forminretur(Request $request)
    {

        $barang = M_returdetailpembelian::where('kdbarang', $request['kdbarang'])->first();
        return view('history.pembelian.forminretur', ['barang' => $barang, 'idrecord' => $request['idrecord']]);
    }
    public function invoiceretur()
    {
        ...
        return view('history.pembelian.invoiceretur')->with(['retur' => $retur, 'detailretur' => $detailretur]);
    }
